...
public:
    template<class URBG>
    explicit CachedObjectiveFunction(const HashFunction& reference_function, uint64_t num_samples, URBG&& urbg)
    {
        ...
        else if (num_samples >= max_input / 2)
        {
            ...
            std::sample(CountingIterator{0}, CountingIterator{max_input}, values.begin(), num_samples, std::forward<URBG>(urbg));
            for (auto& val : values)
            {
    ...
    uint64_t operator()(const HashFunction& function)
    {
...
{
public:
    UncachedObjectiveFunction(const HashFunction& reference_function, uint64_t num_samples, URBG& urbg)
    ...
    uint64_t operator()(const HashFunction& function)
    {
        ...
        {
            std::vector<uint64_t> values(num_samples);
            std::sample(CountingIterator{0}, CountingIterator{dist.max()}, values.begin(), num_samples, urbg);
            for (auto& val : values)
            {
